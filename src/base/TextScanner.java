...
//      ;
  ...
  public String tokenName(Token t) {
    return (dfa == null) ? DFA.defaultTokenName(t.id()) : dfa.tokenName(t.id());
  }
...
//  }
  ...
  * @return original string, or original string with first and last characters
   ...
   */
  public TextScanner(Reader reader, String sourceDescription, DFA dfa, int skipType) {
    this(dfa, skipType);
...
//  }
  ...
  public void include(Reader r, String description, boolean toFront) {
    ...
    MyReader riNew = new MyReader(r, description, dfa);
...
//      echoLineNumber = 0;
  ...
  public Token peekAt(int offset) {
    ...
    return (Token) tokenQueue.peekAt(Math.min(offset, queued() - 1));
  }
   ...
   */
  public static DArray scanSource(Reader src,
   ...
   * @return String
   */
  public String readWord(boolean mustExist, boolean parseString) {
    if (db) {
      System.out.println("readWord mustEx=" + mustExist + " parseStr=" +
                         parseString);
    }

    final int S_TESTSTARTSTRING = 0,
        S_NOTSTRING = 1,
        S_ESCAPED = 2,
        S_INSTRING = 3,
        S_DONE = 4;
  ...
  public String readWord(boolean mustExist) {
    return readWord(mustExist, false);
  }
...
//    Inf inf = new Inf("splitString", 20000);
...
//  private static final int MAX_FILE_LENGTH = 65536;
  ...
  private static class MyReader
      extends DynamicPushbackReader {
    public MyReader(Reader r, String desc, DFA dfa) {
      super(r);
      if (desc == null) {
        desc = r.toString();
      }
...
//    int lineAtStartOfToken;
   ...
   * the reader.
   */
  private static class StdInReader
