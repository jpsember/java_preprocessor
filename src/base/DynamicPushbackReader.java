 ...
 */
public class DynamicPushbackReader extends PushbackReader {
...
//      stack.append((char)c);
  ...
  public int peek(int offset) throws IOException {
    while (queue.length() <= offset) {
      ...
      if (ch < 0)
        return -1;
      queue.pushInt(ch,false);
    }
    return ((Integer)queue.peekAt(offset)).intValue();
  }
